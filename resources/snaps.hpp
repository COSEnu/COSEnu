...
void NuOsc::output_vsnap(double loc, const int t)
{
    ...
    std::string v_snap_fname = "vsnap_t=" + std::to_string(t) + "_z=" + l + "_.dat";
    ...
    {
        int ij = idx(i, j);
        v_snap_ofstream << std::scientific << vz[i] << "\t"
                        << v_stat->ee[ij] << "\t"
                        << v_stat->xx[ij] << "\t"
                        << v_stat->ex_re[ij] << "\t"
                        << v_stat->ex_im[ij] << "\t"
...
void NuOsc::output_zsnap(const int vmode, const int t)
{
    ...
    std::string z_snap_fname = "zsnap_t=" + std::to_string(t) + "_v=" + std::to_string(vmode) + "_.dat";
    ...
    {
        int ij = idx(i, j);
        z_snap_ofstream << std::scientific << Z[j] << "\t"
                        << v_stat->ee[ij] << "\t"
                        << v_stat->xx[ij] << "\t"
                        << v_stat->ex_re[ij] << "\t"
                        << v_stat->ex_im[ij] << "\t"
