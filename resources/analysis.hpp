...
void NuOsc::averaged_survival_prob(const FieldVar *inF, const Profile *inG0, std::ofstream &surv_prob_ofstream,
                                   const int time)
{
    double num_Pee = 0;
    double num_Pbee = 0;
    ...
    for (int i = 0; i < nvz; i++)
    {
        for (int j = 0; j < nz; j++)
        {
            ...
            dnom_Pee += inG0->G[idx(i, j)] * dz * dv;
            dnom_Pbee += inG0->bG[idx(i, j)] * dz * dv;
        }
    }
